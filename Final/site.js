...
function checkIncompleteConfig() {
  const constants = [
      ChatTitle, FooterAreaTitle, PagebarTitle, 
      AIChatBotName, UserName, Avatar_User_Path, Avatar_AI_Path,
      Button_Send_BackgroundColor, Button_Send_Color, Button_Send_Text
  ];

  return constants.includes("YOUR_DATA");
}

// Redirect if any value is not configured
if (checkIncompleteConfig()) {
...
window.onload = function () {
    ...
    fetch('./constant.js')
        .then(response => {
            if (response.ok) {
                // The file exists, proceed with the code
        ...
        .catch(error => {
            console.error('Error checking constant.js file:', error);
            window.location.href = './Con.html';
        });
};
